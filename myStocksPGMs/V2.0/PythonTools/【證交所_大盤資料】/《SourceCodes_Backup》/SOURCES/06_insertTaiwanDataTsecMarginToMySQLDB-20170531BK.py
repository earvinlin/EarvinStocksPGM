...
#    tsec = open(input_file, 'r', encoding = 'UTF-8') # 以utf-8開啟
    tsec = open(input_file, 'r')	# 預設以系統編碼開啟
    reader = csv.DictReader(tsec, delimiter=';')


#	NOTE : parse 日期 yyy/mm/dd (因為民國年可能為2位數或3位數，所以改由後面算長度)
    theDate = sys.argv[1][0:(len(sys.argv[1])-4)] + "/" + sys.argv[1][(len(sys.argv[1])-4):(len(sys.argv[1])-2)] + "/" + sys.argv[1][(len(sys.argv[1])-2):len(sys.argv[1])]
    print("交易日期= " + theDate)
    tehDateList = theDate.split("/")
    theTradeDate = datetime.date((int(tehDateList[0])+1911), int(tehDateList[1]), int(tehDateList[2]))  # 轉西元日期
