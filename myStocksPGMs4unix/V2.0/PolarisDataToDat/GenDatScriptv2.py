...
try:
	if len(sys.argv) < 2 :
		...
		print("syntax : C:\python3 GenDatScriptv2.py 20250706")
		sys.exit()
	counts = 0
	find_YMD = sys.argv[1]
	DATA_DIR = "csv/"
	outputFile = "__GenStocksDatScript.sh"
...
#			transFile2dat.exe 4126
			outfile.write("Python3 FormatFile.py " + filename + " " + stockNo + ".txt\n")
			outfile.write("Python3 InsertStocksFromPolarisToMySQLDB.py " + stockNo + " " + stockNo + ".txt\n")
			outfile.write("Python3 GetFileFromDBv2.py " + stockNo + "\n")
			outfile.write("./ux_transFile2dat " + stockNo + "\n")
			outfile.write("\n")
			counts += 1
