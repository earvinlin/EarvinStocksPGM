...
#   Windows預設為CP950編碼 (在Windows會跳行<應該是windows 與 unix的換行符號不同所致!!>)  
    with open(saveFileDir + input_file, 'r', encoding = 'CP950') as infile :
        for each_line in infile :   
            ...
            print(str(totalCnt) + ", len(theInData) = " + str(len(theInData)))
            if len(theInData) != 9 : 
                theStockData = []
                for s in theInData :
                    if len(s) > 0 : 
...
#---------- process about db section END ---------------------------------------
    print('資料處理完成!!共' + str(totalCnt) + '筆。') 
    print("Insert Count: " + str(insertCnt))
    print("Duplicate Count: " + str(insertCnt))
    print("Error Count: " + str(insertCnt))
