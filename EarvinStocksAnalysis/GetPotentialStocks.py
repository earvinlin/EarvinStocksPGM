"""
EarvinStocksAnalysis.GetPotentialStocks 的 Docstring
SQL-CMD:
select a.股票代號, d.stock_name as 股票名稱, a.年度, a.累計營收_億, 
a.累計營收年增_百分比, b.股本_億, b.財報評分, b.年度股價_收盤, b.年度股價_平均, 
b.roe, b.roa, b.bps, c.股利所屬期間, c.股利合計, c.年均殖利率_合計, c.eps, 
c.盈餘分配率_合計 
from ( 
  select stock_no as 股票代號, mid(cast(date as char),1,4) as 年度, 
 		cum_revenue as 累計營收_億, ann_ins_cum_revenue_p as 累計營收年增_百分比 
 	from stocks_sale_month where mid(cast(date as char),5,2) = '12' 
) a 
left outer join ( 
	select stock_no as 股票代號, year as 年度, share_capital as 股本_億, 
  	fin_report_score as 財報評分, ann_stock_end_price as 年度股價_收盤, 
  	ann_stock_avg_price as 年度股價_平均, roe, roa, bps 
  from stocks_bz_performance 
) b 
on a.股票代號 = b.股票代號 and a.年度 = b.年度 
left outer join ( 
	select stock_no as 股票代號, period_of_dividend as 股利所屬期間, 
  	total_dividend as 股利合計, year_high_price as 年度股價最高價, 
   	year_low_price as 年度股價最低價, year_avg_price as 年度股價年均價, 
...
where a.股票代號 = '1101' order by a.年度 desc ; 
"""
